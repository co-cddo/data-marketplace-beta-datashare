...
[TestFixture]
public class TimeValueHasNotBeenSuppliedValidationRuleTests
{
    ...
    public void GivenANullValidationRule_WhenICheckAccepts_ThenAnArgumentNullExceptionIsThrown()
    {
        ...
        Assert.That(() => testItems.TimeValueHasNotBeenSuppliedValidationRule.Accepts(null!),
            Throws.ArgumentNullException.With.Property("ParamName").EqualTo("validationRule"));
    }

    [Test]
    public void GivenAValidationRuleWithTimeValueHasNotBeenSuppliedRuleId_WhenICheckAccepts_ThenTrueIsReturned()
    {
        var testItems = CreateTestItems();

        var testValidationRule = testItems.Fixture.Build<QuestionPartAnswerValidationRuleModelData>()
            .With(x => x.QuestionPartAnswerValidationRule_Rule, QuestionPartAnswerValidationRuleId.FreeForm_Time_NoValueSupplied)
            .Create();

        var result = testItems.TimeValueHasNotBeenSuppliedValidationRule.Accepts(testValidationRule);

        Assert.That(result, Is.True);
    }

    [Test]
    public void GivenAValidationRuleNotWithTimeValueHasNotBeenSuppliedRuleId_WhenICheckAccepts_ThenFalseIsReturned(
    ...
    private static IEnumerable<QuestionPartAnswerValidationRuleId> NonApplicableValidationRuleIds =>
        Enum.GetValues<QuestionPartAnswerValidationRuleId>().Except([QuestionPartAnswerValidationRuleId.FreeForm_Time_NoValueSupplied]);
    #endregion
    ...
    public void GivenANullQuestionAnswerPart_WhenICheckWhetherResponseFailsValidation_ThenAnArgumentNullExceptionIsThrown()
    {
        ...
        Assert.That(() => testItems.TimeValueHasNotBeenSuppliedValidationRule.ResponseFailsValidation(null!),
    ...
    public void GivenANonFreeFormOptionResponse_WhenICheckWhetherResponseFailsValidation_ThenAnInvalidOperationExceptionIsThrown()
    {
        ...
        Assert.That(() => testItems.TimeValueHasNotBeenSuppliedValidationRule.ResponseFailsValidation(questionAnswerPartResponse),
            Throws.InvalidOperationException.With.Message.EqualTo("QuestionAnswerPartResponse is not of expected type for validation rule TimeValueHasNotBeenSuppliedValidationRule"));
    }
        ...
        [Values("", "  ")] string enteredValue)
    {
        ...
        var result = testItems.TimeValueHasNotBeenSuppliedValidationRule.ResponseFailsValidation(questionAnswerPartResponse);
    ...
    private static TestItems CreateTestItems()
    {
        ...
        var timeValueHasNotBeenSuppliedValidationRule = new TimeValueHasNotBeenSuppliedValidationRule();

        return new TestItems(
            fixture,
            timeValueHasNotBeenSuppliedValidationRule);
    }

    private class TestItems(
        IFixture fixture,
        IValidationRule timeValueHasNotBeenSuppliedValidationRule)
    {
        public IFixture Fixture { get; } = fixture;
        public IValidationRule TimeValueHasNotBeenSuppliedValidationRule { get; } = timeValueHasNotBeenSuppliedValidationRule;
    }
    #endregion
}